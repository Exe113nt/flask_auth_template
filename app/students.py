...
def viewstudent(student_id):
    ...
    query = f"select group.name from group join student on student.group_id=group.id where student.id={student_id}"
    # group = db.engine.execute(query).first().text
    route = '/studentslist'
    return render_template('viewstudent.html', student=student, route=route)
...
def editstudent_post():
    ...
    student = Student.query.filter_by(id=student_id).update({'name':name,
                                                            'surname':surname,
                                                            'group_id':Group.query.filter_by(text=group).first().id})
    db.session.commit()
...
def newstudent_post():
    ...
    new_student = Student(name=name, surname=surname, group_id=Group.query.filter_by(text=group).first().id, couch_id=current_user.id)
